...
new class extends Component
{
    ...
    public string $sortField = 'attributed_revenue';
    ...
    {
        $allowed = [
            'name', 'sent', 'opens', 'clicks', 'unique_clicks',
            'platform_revenue', 'sent_at',
            'attributed_revenue', 'rev_per_click', 'ecpm',
        ];
    ...
    public function with(): array
    {
        ...
        $query = CampaignEmail::query()
            ...
            ->where('campaign_emails.workspace_id', $workspace->id)
            ->whereBetween('campaign_emails.sent_at', [$this->start, Carbon::parse($this->end)->endOfDay()])
            ->select([
                'campaign_emails.*',
                DB::raw('COALESCE(attr.attributed_revenue, 0) as attributed_revenue'),
                DB::raw('CASE WHEN campaign_emails.unique_clicks > 0 THEN COALESCE(attr.attributed_revenue, 0) / campaign_emails.unique_clicks ELSE 0 END as rev_per_click'),
                DB::raw('CASE WHEN campaign_emails.opens > 0 THEN COALESCE(attr.attributed_revenue, 0) / campaign_emails.opens * 1000 ELSE 0 END as ecpm'),
            ]);

        if ($this->search) {
            $query->where(function ($q) {
                $q->where('campaign_emails.name', 'like', "%{$this->search}%")
                    ->orWhere('campaign_emails.subject', 'like', "%{$this->search}%");
            });
        }

        $nativeColumns = ['name', 'sent', 'opens', 'clicks', 'unique_clicks', 'platform_revenue', 'sent_at'];
        $sortColumn = in_array($this->sortField, $nativeColumns)
            ? "campaign_emails.{$this->sortField}"
            : $this->sortField;
...
<x-layouts.app>
    <x-slot:title>Campaign Revenue</x-slot:title>

    @volt('reports.campaign-revenue')
    <div>
        <div class="flex justify-between items-start mb-6">
            <div>
                <h1 class="text-[22px] font-bold text-slate-900 dark:text-white">Campaign Revenue</h1>
                <p class="text-[13px] text-slate-600 dark:text-slate-300 mt-0.5">Campaign emails with attributed revenue, Rev/Click, and ECPM (last-click attribution)</p>
            </div>
        </div>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-slate-200 dark:border-slate-700">
                                ...
                                <th class="text-[10.5px] font-semibold uppercase tracking-[0.4px] text-slate-400 px-3 py-[11px] cursor-pointer" wire:click="sort('attributed_revenue')">Attr. Revenue</th>
                                <th class="text-[10.5px] font-semibold uppercase tracking-[0.4px] text-slate-400 px-3 py-[11px] cursor-pointer" wire:click="sort('rev_per_click')">Rev/Click</th>
                                <th class="text-[10.5px] font-semibold uppercase tracking-[0.4px] text-slate-400 px-3 py-[11px] cursor-pointer" wire:click="sort('ecpm')">ECPM</th>
                        ...
                        <tbody class="text-[12.5px]">
                            @foreach ($campaigns as $campaign)
                                <tr class="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                                    ...
                                    <td class="px-3 py-2.5 font-mono font-medium {{ $campaign->attributed_revenue > 0 ? 'text-emerald-600 dark:text-emerald-400' : '' }}">${{ number_format($campaign->attributed_revenue, 2) }}</td>
                                    <td class="px-3 py-2.5 font-mono">${{ number_format($campaign->rev_per_click, 2) }}</td>
                                    <td class="px-3 py-2.5 font-mono">${{ number_format($campaign->ecpm, 2) }}</td>
